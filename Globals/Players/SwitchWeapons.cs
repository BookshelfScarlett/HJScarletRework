...
namespace HJScarletRework.Globals.Players
{
    public partial class HJScarletPlayer : ModPlayer
    {
        ...
        public void SwitchWeaponSystem()
        {
            ...
            {
                ReplaceWeaponsOnNeed(targetWeapons);
                return;
            }
            int reverseWeapon = GetReverseWeapon(ownerItemType);
            if (reverseWeapon != -1)
            {
                ReplaceWeaponsOnNeed(reverseWeapon);
                return;
            }
        }
        private void ReplaceWeaponsOnNeed(int targetWeapons)
        {
            Item targetWeapon = new Item();
            targetWeapon.SetDefaults(targetWeapons);
            //继承词缀
            targetWeapon.Prefix(Player.HeldItem.prefix);
            //直接……干掉玩家的物品
