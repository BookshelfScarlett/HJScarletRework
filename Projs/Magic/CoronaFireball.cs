    ...
    public class CoronaFireball : HJScarletProj, IPixelatedRenderer
    {
        ...
        public override void ProjAI()
        {
            ...
            else
            {
                ...
                {
                    if (Projectile.velocity.LengthSquared() >= 0.01f * 0.01f)
                        Projectile.velocity *= 0.9f;
                }
            }
        ...
        public void DrawTrails(Asset<Texture2D> useTex, Color drawColor, float multipleSize = 1f, float alphaValue = 1f, float offsetHeight = 1f)
        {
            ...
            for (int j = 0; j < posCount - 1; j++)
            {
                ...
                trailDrawDates.Add(new(validPosition[j] + Projectile.Size / 2, drawColor, new Vector2(0, 35 * multipleSize * Projectile.scale), rot));
            }
