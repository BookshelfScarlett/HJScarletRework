...
{
    public class FleshtumorHeldProj : HJScarletProj
    {
        public override EnumDamageClass Category => EnumDamageClass.Magic;
        public int AttackSpeed => Owner.ApplyWeaponAttackSpeed(GetInstance<Fleshtumor>().Item, GetInstance<Fleshtumor>().Item.useTime * Projectile.MaxUpdates, 5 * Projectile.MaxUpdates);
        ...
        public override void ProjAI()
        {
            ...
            if (Owner.HeldItem.type != ItemType<Fleshtumor>())
            {
        ...
        private void HoldIdleState()
        {
            ...
            bool reverse = !Helper.IsDone[0] && Owner.HeldItem.type != ItemType<Fleshtumor>() || Owner.controlUseTile;
            ...
            else
            {
                ...
                if (Timer > AttackSpeed / 5 && Helper.IsDone[1])
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), pos, vel, ProjectileType<FleshtumorHunger>(), Projectile.damage, Projectile.knockBack, Owner.whoAmI);
                    }
                    for (int i = 0; i < 8; i++)
                    {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ECSParticle.SmokeParticle(Projectile.Center.ToRandCirclePos(6f) + Projectile.SafeDir() * 0f, RandVelTwoPi(0f, 4f), RandLerpColor(Color.DarkRed, Color.Crimson), 40, RandRotTwoPi, 1f, 0.20f * Main.rand.NextFloat(0.50f, 1.1f), Main.rand.NextBool(), BlendState.NonPremultiplied);
                    }
                    for (int i = 0; i < 4; i++)
                    {
                        ECSParticle.HRShinyOrb(Projectile.Center.ToRandCirclePos(6), RandVelTwoPi(0, 4), RandLerpColor(Color.DarkRed, Color.Crimson), 40, 1, .2f * Main.rand.NextFloat(.5f, 1.1f) * .15f, 0f, BlendState.NonPremultiplied);
                    }
                }
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < 16; i++)
                SB.Draw(tex, realDrawPos + (TwoPi / 16f * i).ToRotationVector2() * 1.2f * progress2, null, Color.Red.ToAddColor() * progress2, rotation, origin, Projectile.scale * progress2, se, 0);
            ...
            SB.Draw(ring, realDrawPos, null, Color.DarkRed * .5f * progress2, Projectile.rotation, ring.ToOrigin(), ringScale * 2f, 0, 0);
