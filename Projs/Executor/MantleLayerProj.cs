    ...
    public class MantleLayerProj : HJScarletProj
    {
        ...
        private void DoReadyHeavyHit()
        {
            ...
            if (CurTarget != null && CurTarget.CanBeChasedBy())
            {
                ...
                else
                {
                    ...
                    Projectile.rotation = Projectile.rotation.AngleLerp(Projectile.velocity.ToRotation(), 0.08f);
        ...
        private void UpdateParticle()
        {
            if (Projectile.IsOutScreen())
                return;
            bool isState = Owner.HasProj<MantleLayerExecution>() && Main.rand.NextBool() && (AttackType == State.Hit || AttackType == State.ReadyHeavyHit);
            if (isState)
                return;
            ...
            if (Projectile.FinalUpdateNextBool(3) && Projectile.HJScarlet().ExecutionStrike)
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.AddExecutionTimeImmediate(ItemType<MantleLayer>());
            ...
            switch (AttackType)
            {
                ...
                case State.ReadyHeavyHit:
                    ...
                    bool directReturn = CurTarget == null || Owner.HasProj<MantleLayerExecution>();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //我的天哪这个转角属于是让我吃了好大一股
            //这里这个转角都是为了让锤头对准敌人
            //因为只有回击的才需要，为了撞击之后的consisttency，这里是全局改的
            //反正其他情况下这个射弹一直在自转
            SpriteEffects se = Projectile.spriteDirection > 0 ? SpriteEffects.FlipVertically : 0;
            float rotSe = Projectile.spriteDirection > 0 ? -PiOver2 : -PiOver4;

            for (int i = Projectile.oldPos.Length - 1; i >= 0; i--)
            {
                float ratios = ((float)i / (Projectile.oldPos.Length - 1));
                Color lerpColor = Color.Lerp(Color.White, Color.Orange, ratios);
                SB.Draw(tex, Projectile.oldPos[i] + Projectile.PosToCenter(), null, lerpColor * (1 - ratios), Projectile.oldRot[i] + rotSe, tex.ToOrigin(), Projectile.scale, se, 0);
            }
            return false;
        }
    }
