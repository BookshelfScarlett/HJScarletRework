    ...
    public class TheDefilerProj : HJScarletProj
    {
        ...
        private void CanFocusStrike()
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                new SmokeParticle(Projectile.Center.ToRandCirclePos(6f), dVel, RandLerpColor(Color.DarkGreen, Color.SeaGreen), 40, RandRotTwoPi, 1f, 0.20f * Main.rand.NextFloat(0.50f, 1.1f), RandBoolen()).SpawnToNonPreMult();
            }
            for (int i = 0; i < 16; i++)
            {
                Vector2 dVel = vel.ToRandVelocity(ToRadians(10f)) * Main.rand.NextFloat(4f, 18f);
                new ShinyOrbHard(Projectile.Center.ToRandCirclePos(6f), dVel, RandLerpColor(Color.DarkViolet, Color.Purple), 40, Main.rand.NextFloat(0.4f, 0.8f)).SpawnToNonPreMult();
            }
        }
        ...
        private void TileCollideParticle(Vector2 velo)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                new SmokeParticle(pos, dVel, RandLerpColor(Color.DarkGreen, Color.SeaGreen), 40, RandRotTwoPi, 1f, 0.20f * Main.rand.NextFloat(0.50f, 1.1f), RandBoolen()).SpawnToNonPreMult();
            }
            for (int i = 0; i < 8; i++)
            {
                ...
                new ShinyOrbHard(pos, dVel, RandLerpColor(Color.DarkViolet, Color.Purple), 40, Main.rand.NextFloat(0.4f, 0.8f)).SpawnToNonPreMult();
            }
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 24; i++)
            {
                new Fire(target.Center.ToRandCirclePos(4f), RandVelTwoPi(6f), RandLerpColor(Color.DarkViolet, Color.Purple), 40, RandRotTwoPi, 1f, 0.1f).SpawnToNonPreMult();
            }
            for (int i = 0; i < 24; i++)
            {
                new ShinyOrbHard(target.Center.ToRandCirclePos(6f), RandVelTwoPi(4f), RandLerpColor(Color.DarkGreen, Color.SeaGreen), 40, Main.rand.NextFloat(0.4f, 0.8f)).SpawnToNonPreMult();
            }
        }
        public override bool PreDraw(ref Color lightColor)
        {
            Projectile.DrawGlowEdge(Color.DarkViolet);
