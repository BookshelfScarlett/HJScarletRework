...
{
    public class JudgementStarNormal : HJScarletProj, IPixelatedRenderer
    {
        ...
        public void UpdateAniProgress()
        {
            ...
            {
                case State.OnNormal:
                    UpdateNormalAIProgress();
                    break;
                case State.OnExecution:
                    UpdateOnExecutionAIProgress();
                    break;
            }
        }

        public void UpdateOnExecutionAIProgress()
        {
            if (!Helper.IsDone[0])
            {
            ...
            else
            {
                if (TargetNPC != null && TargetNPC.CanBeChasedBy())
                    Projectile.HomingTarget(TargetNPC.Center, -1, 16f, 16f);
                else
                    CanFadeAway = true;
            }
        }

        public void UpdateNormalAIProgress()
        {
            if (!Helper.IsDone[0])
            {
                Helper.UpdateAniState(0);
                Projectile.scale = EaseOutExpo(Helper.GetAniProgress(0));
                Projectile.velocity *= 0.95f;
                float rotValue = ToRadians(2f);
                if (Main.rand.NextFloat(0.6f, 1f) > Helper.GetAniProgress(0))
                    rotValue = -ToRadians(2f);
                Projectile.velocity = Projectile.velocity.RotatedBy(rotValue);
            }
            else if (!CanFadeAway)
            {
                if (TargetNPC != null && TargetNPC.CanBeChasedBy())
                    Projectile.HomingTarget(TargetNPC.Center, -1, 16f, 20f);
                else
                    CanFadeAway = true;
            }
            else
        ...
        public override bool? CanHitNPC(NPC target)
        {
            ...
            if (canHit || AttackState == State.OnExecution)
                return null;
            return false;
        }
