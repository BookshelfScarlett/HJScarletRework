...
namespace HJScarletRework.Projs.Executor
{
    public class SpectreStaffHeldProj : ExecutorHeldProj
    {
        ...
        public void UpdateAttack()
        {
            ...
            Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), spawnPos, Projectile.velocity.ToRandVelocity(ToRadians(0f), 12f, 18f), ProjectileType<SpectreStaffGhost>(), Projectile.damage, 2f, Owner.whoAmI);
            proj.extraUpdates = 2;
            for (int i = 0; i < 2; i++)
            {
                proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), spawnPos, Projectile.velocity.ToRandVelocity(ToRadians(10f), 12f, 18f), ProjectileType<SpectreStaffGhost>(), Projectile.damage, 2f, Owner.whoAmI);
                proj.extraUpdates = 2;
                for (int j = 0; j < 6; j++)
        ...
        public bool CheckOwnerDead()
        {
            ...
            bool doAFK = Helper.IsDone[0] && Helper.IsDone[1] && ifDead && !IsAFK;
            if(doAFK)
            {
        ...
        public override bool ShouldUpdatePosition() => IsAFK;
