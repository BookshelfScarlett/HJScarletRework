...
namespace HJScarletRework.Projs.Executor
{
    public class TheJudgementCrossMounted : HJScarletFriendlyProj, IPixelatedRenderer
    {
        ...
        private void SpawnHolyJudement()
        {
            ...
            ScreenDarknessSystem.AddScreenDarkness(0.85f, 6, 1, 45, easeOut: EaseInCubic);
            for (int i = 0; i < 4; i++)
            {
                //666我还要存他们的数组信息来着
        ...
        private void DoExta()
        {
            ...
            AttackTimer += 1f;
            if (AttackTimer > 10f && Owner.ownedProjectileCounts[ProjectileType<TheJudgementCross>()] < 1)
                Projectile.Kill();
        }
