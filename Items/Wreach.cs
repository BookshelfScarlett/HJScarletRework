...
namespace HJScarletRework.Items
{
    public class Wreach : HJScarletWeapon
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            //    }
            //}
            Projectile proj = Projectile.NewProjectileDirect(source, position, velocity.ToSafeNormalize() * 10f, ProjectileType<ContainedBlastBoom>(), 123, knockback, player.whoAmI);
            sw.Stop();
            // 输出经过的时间（毫秒）
