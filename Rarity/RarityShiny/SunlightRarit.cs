...
{
    public class SunlightRarity : ModRarity
    {
        ...
        public static void PostDrawRarity(ref List<RaritySparkle> particleList, DrawableTooltipLine tooltipLine)
        {
            ...
            if (Main.rand.NextBool(10))
            {
                ...
                int lifetime = 160;
                Vector2 position = GetParticlePosition(tooltipLine);
                Vector2 velocity = -Vector2.UnitY * Main.rand.NextFloat(0.25f, 0.45f);
            ...
            if (Main.rand.NextBool(10))
            {
                ...
                int lifetime = 160;
                Vector2 position = GetParticlePosition(tooltipLine);
                Vector2 velocity = -Vector2.UnitY * Main.rand.NextFloat(0.25f, 0.30f);
                RarityShinyOrb rarityShinyOrb = new RarityShinyOrb(position, velocity, RandLerpColor(Color.DarkOrange, Color.OrangeRed).ToAddColor(), lifetime, scale);
                position = GetParticlePosition(tooltipLine);
                velocity = -Vector2.UnitY * Main.rand.NextFloat(0.25f, 0.30f);
                scale = Main.rand.NextFloat(0.30f * 0.5f, 0.30f) * 1.2f;
                RarityShinyOrb rarityShinyOrb2 = new RarityShinyOrb(position, velocity, RandLerpColor(Color.DarkOrange, Color.OrangeRed).ToAddColor(), lifetime, scale);
                particleList.Add(rarityShinyOrb);
                particleList.Add(rarityShinyOrb2);
