...
namespace HJScarletRework.Rarity
{
    public class SakuraRarity : ModRarity
    {
        ...
        public static void DrawCustomTooltipLine(DrawableTooltipLine tooltipLine)
        {
            ...
            Main.spriteBatch.Draw(HJScarletTexture.Texture_RarityGlow.Value, glowPosition, null, Color.HotPink.ToAddColor() * 0.85f, 0f, HJScarletTexture.Texture_RarityGlow.Origin, glowScale, SpriteEffects.None, 0f);
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                ChatManager.DrawColorCodedString(Main.spriteBatch, tooltipLine.Font, textValue, (textPosition + afterimageOffset).RotatedBy(TwoPi * (i / 12)), Color.White* 0.9f, tooltipLine.Rotation, tooltipLine.Origin, tooltipLine.BaseScale);
            }

            // Draw the main inner text.
            Color mainTextColor = Color.Lerp(Color.HotPink, Color.DeepPink, 0.9f);
